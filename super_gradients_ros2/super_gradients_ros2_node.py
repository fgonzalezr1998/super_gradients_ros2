...
class SuperGradientsNode(Node):
    ...
    def _publish_bboxes(self, predictions, raw_img):
        ...
        for pred in predictions:
            ...
            for i, (label, conf, bbox) in enumerate(zip(labels, confidence, bboxes)):
                ...
                detection.bbox.center.x = (bbox[2].item() + bbox[0].item()) /  2.0
                detection.bbox.center.y = (bbox[3].item() + bbox[1].item()) / 2.0
                detection.bbox.size_x = bbox[2].item() - bbox[0].item()
                detection.bbox.size_y = bbox[3].item() - bbox[1].item()
    ...
    def _step(self):
        if (self._status.is_ok()):
            if (self._image_received):
                ...
                img = self._cv_bridge.imgmsg_to_cv2(self._last_img_msg)
                img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

                predictions = self._model.predict(img, conf=self._min_prob)

                self._publish_bboxes(predictions, raw_img)
        else:
